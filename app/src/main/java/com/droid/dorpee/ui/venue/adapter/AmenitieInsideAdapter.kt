...
class AmenitieInsideAdapter(internal val context: Context, var list: List<Data>?, val services: List<Service>?) :
        RecyclerView.Adapter<AmenitieInsideAdapter.ViewHolder>() {
...
//       val lines = holder.binding.checkboxId.lineCount
        val params = LinearLayoutCompat.LayoutParams(
                LinearLayoutCompat.LayoutParams.MATCH_PARENT, context.resources.getDimension(
                R.dimen._35sdp
        ).toInt())
        services?.map {
            if (it.id == item.id){
                list!![pos].checked = true
                RegisterVenueActivity.listOfAmenitie?.add(list!![pos])
            }
        }
