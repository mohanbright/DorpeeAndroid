...
import com.droid.dorpee.ui.search.ui.booking.BookingVenueFragment
import com.droid.dorpee.ui.venuedashboard.adapter.TodayVenueAdapter
...
class TodayVenueBookingFrag : BaseFragment() {
    ...
    override fun onCreateView(
        ...
        todaysBoking = arguments?.getSerializable("data") as ArrayList<VenueBooking>?
        return binding.root
    }
    ...
    fun attachObserver() {
        viewModel?.apiBookingChangeStatus?.observe(viewLifecycleOwner, Observer { v ->
            showProgress(false)
            val fragment = parentFragmentManager.findFragmentById(R.id.nav_host_fragment) as BookingVenueFragment
            fragment.viewModel?.getVenueDashboard(RequestWithHeader(AppPreferences.getInstance(requireContext())?.user?.token))
            todaysBoking?.map {
                if (v.id == it.id) {
                    v
                } else {
            ...
            /*(binding.recyclerUpcoming.adapter as UpcomingVenueAdapter).setData(todaysBoking?:
        ...
        this.viewModel?.apiError?.observe(viewLifecycleOwner, Observer {
            showProgress(false)
            val fragment = parentFragmentManager.findFragmentById(R.id.nav_host_fragment) as BookingVenueFragment
            fragment.showUpSnackError(it)
        })

    }

    private fun setUpTodayBooking() {
        if (todaysBoking.isNullOrEmpty()) {
            ...
            binding.recyclerUpcoming.adapter =
                    TodayVenueAdapter(requireContext(), todaysBoking ?: ArrayList())
            (binding.recyclerUpcoming.adapter as TodayVenueAdapter).itemClick =
